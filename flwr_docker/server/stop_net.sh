...
DESTINATION_DIR="/home/tshark_captures"
...
# Kill TShark process and save report
...
if [ -f "$CAPTURE_FILE_PATH" ]; then
    ...
    cp "$CAPTURE_FILE_PATH" "$DESTINATION_DIR/"
    ...
    tshark -r "$FINAL_PCAP_PATH" -q -z conv,tcp | LC_NUMERIC=C awk '
    BEGIN {
        OFS=",";
    ...
    chown -R "$REAL_USER:$REAL_USER" "$DESTINATION_DIR"
