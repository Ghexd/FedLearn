...
def load_data_from_disk(path: str, batch_size: int):
    ...
    # Define the transformation to normalize the data to the range [-1, 1] (assuming dataset has already scaled the data to [0, 1])
...
def train(net, trainloader, epochs, learning_rate, device):
    ...
    for epoch in range(epochs):
        ...
        stop_time = time.time()
        epoch_duration = stop_time - start_time
        ...
        try:
            ...
            with open(train_file_path, "a") as file:
                file.write(f"Epoch {epoch+1}: Time {epoch_duration:.4f}s, Loss {avg_epoch_loss:.4f}\n")
        except IOError as e:
    ...
    try:
        ...
        with open(train_file_path, "a") as file:
            file.write(f"Mean train time: {mean_elapsed_time:.4f} seconds, Mean Loss: {avg_trainloss:.4f}\n\n")
    except IOError as e:
        print(f"Error writing file: {e}")
...
def test(net, testloader, device):
    ...
    with torch.no_grad():
        for batch in testloader:
            ...
            stop_time = time.time()
            batch_duration = stop_time - start_time
            ...
            try:
                ...
                with open(test_file_path, "a") as file:
                    file.write(f"Batch n.{ind}: {batch_duration:.4f} seconds\n")
            except IOError as e:
                print(f"Error writing file: {e}")
    ...
    try:
        ...
        with open(test_file_path, "a") as file:
            file.write(f"Mean test time: {mean_elapsed_time:.4f} seconds, Mean Loss: {loss:.4f}, Accuracy: {accuracy:.4f}\n\n")
    except IOError as e:
        print(f"Error writing file: {e}")
